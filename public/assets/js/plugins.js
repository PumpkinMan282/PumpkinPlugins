...
function mdToHtml(markdown){
  ...
  flushList();
  flushCode();
  return html || "<p>No guide available yet.</p>";
}
...
async function renderDetail({plugin}){
  ...
  const summary =
    plugin?.summary?.trim() ||
    "No summary yet. Add one in public/data/plugins.json.";
  ...
  const guide = $("#guide");
  if (!plugin?.guidePath){
    guide.innerHTML = "<p>No guide available yet.</p>";
    return;
  }
  try{
    ...
    guide.innerHTML = `<p>Couldn’t load this guide yet. (${escHtml(e?.message || "error")})</p>`;
  }
}
